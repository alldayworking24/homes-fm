...
function exportRoomCheckExcel(){
  ...
  recs.forEach(r=>{
    ...
    items.forEach((it,itemIndex)=>data.push({
      ...
      '차기 세입자 입주 예정일':r.nextTenantMoveInDate||''
    }));
  });
...
function nextTenantMoveInHtml(rec){if(!rec?.nextTenantMoveInDate)return'';return `<div class="moveInLine">${uiIcon('calendar')}<span>차기 세입자 입주 예정일 · ${esc(rec.nextTenantMoveInDate)}</span></div>`}
...
function renderRepairList(){
  ...
  repairWrap.innerHTML=recs.map(r=>{
    ...
    return `<div class="list">
      ...
      <div class="repchip ${doneN===need.length?'done':''}">보수 ${doneN}/${need.length} 완료</div>${repairDeadlineHtml(r)}${nextTenantMoveInHtml(r)}${bearerHtml}</div>
...
function openRepairDetail(id,viewOnly=false){
  ...
  const deadlineInfo=repairDeadlineInfo(rec.repairDeadline,false);repairDetailWrap.innerHTML=`<div class="repairDeadlineBox"><div class="repairDeadlineField"><div class="repairDeadlineHead"><label class="repairDeadlineTitle" for="repairDeadline"><span data-inline-icon="calendar"></span><span>보수 완료 기한 <small>(선택)</small></span></label><span id="repairDeadlineBadge" class="deadlineBadge ${deadlineInfo?.cls||''}">${deadlineInfo?.text||'미설정'}</span></div><input id="repairDeadline" type="date" min="${localTodayYmd()}" value="${esc(rec.repairDeadline||'')}" onclick="openDatePicker(this)" oninput="updateRepairDeadlineBadge(this)" aria-label="보수 완료 기한 달력 열기"></div><div class="repairDeadlineField moveInField"><div class="repairDeadlineHead"><label class="repairDeadlineTitle" for="nextTenantMoveInDate"><span data-inline-icon="calendar"></span><span>차기 세입자 입주 예정일 <small>(선택)</small></span></label></div><input id="nextTenantMoveInDate" type="date" min="${localTodayYmd()}" value="${esc(rec.nextTenantMoveInDate||'')}" onclick="openDatePicker(this)" aria-label="차기 세입자 입주 예정일 달력 열기"><div class="tenantDateWarn">입주 예정일이 있으면 보수 우선순위 판단에 사용합니다.</div></div></div>`+(need.length?need.map(({it,idx})=>`<div class="repItem ${it.repairDone?'done':''}" data-idx="${idx}"><div class="repHead"><div><b>${it.name}</b><div class="sub">${it.category} · ${it.sub||''}</div></div><span class="chip" style="color:${SCOL[it.status]};background:${SBG[it.status]}">${SLAB[it.status]}</span></div>
...
async function saveRepair(){
  ...
  const deadlineInput=document.getElementById('repairDeadline');if(deadlineInput?.value&&deadlineInput.value<localTodayYmd()){alert('보수 완료 기한은 오늘 이후 날짜만 선택할 수 있습니다.');deadlineInput.focus();if(saveBtn){saveBtn.disabled=false;saveBtn.innerHTML=originalLabel;}return}
  const nextTenantInput=document.getElementById('nextTenantMoveInDate');if(nextTenantInput?.value&&nextTenantInput.value<localTodayYmd()){alert('차기 세입자 입주 예정일은 오늘 이후 날짜만 선택할 수 있습니다.');nextTenantInput.focus();if(saveBtn){saveBtn.disabled=false;saveBtn.innerHTML=originalLabel;}return}
  ...
  try{
    ...
    rec.nextTenantMoveInDate=document.getElementById('nextTenantMoveInDate')?.value||'';
